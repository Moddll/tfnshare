...
class GetStock:
    ...
    def get_price_m(self):
        ...
        for items in self.__symbol:
            stockdata = pdr.get_data_yahoo(items, self.__start_date, self.__end_date, threads=20)
            # print(stockdata)
...
class FormatStockData:
    ...
    def ma_stockdata(self, exchange, symbol):

        # dbc = RwDatabase(self._db)
        #
        # sqlflt_str = "select * from " + symbol + " Order By Date"
        # sql_data = dbc.read_sqldata(sqlflt_str)
        sql_data = data_manager.get_data(exchange, symbol)

        df = sql_data
        # df['date'] = sql_data.index
